...
public class Loop : MonoBehaviour
{
    ...
    {
        Collider[] col = Physics.OverlapSphere(transform.position, m_InnerRadius, m_ThoughtLayer);
        if (col != null && col.Length > 0)
        {
            for (int i = 0; i < col.Length; i++)
            {
                Projectile proj = col[i].GetComponent<Projectile>();
                if (Mathf.Sign(proj.m_PositivityDamage) != Mathf.Sign(m_PositiveNegative))
                {
